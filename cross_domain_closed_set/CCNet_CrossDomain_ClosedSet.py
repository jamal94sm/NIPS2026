"""
CCNet — Cross-Domain Closed-Set Evaluations on Palm-Auth
=========================================================
...
CONFIG = {
    ...
    # Misc
    "base_results_dir"     : "./rst_ccnet_crossdomain",
    "random_seed"          : 42,
    "save_every"           : 50,
    "eval_every"           : 50,
    "num_workers"          : 4,
...
def parse_setting_leave_one_condition(
    ...
    No splitting — the full training domain is the gallery;
    the full test condition is the probe.
    """
...
def print_and_save_summary(all_results, out_path):
    ...
    header = (f"{'Setting':<22}"
              ...
              f"{'Rank-1 (%)':>{col_w}}")
    sep = "─" * len(header)
    lines = ["\nCross-Domain Closed-Set Results — Palm-Auth (CCNet)", sep, header, sep]
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"  CCNet — Cross-Domain Closed-Set (Palm-Auth)")
