...
CONFIG = {
    ...
    'pgd_steps_geo'      : 1,
    'pgd_steps_tex'      : 2,
    # FIX B: α sampled from N(0.1, 0.001) per optimize() call — paper §4.3
    # (no fixed pgd_step_size — sampling is inside AdversarialAugOptimizer)
    ...
    # ── Augmentation rate ─────────────────────────────────────────────────
    # FIX C: γ = fraction of each batch to augment — paper §4.3: γ=0.5
    'aug_gamma'          : 0.5,

    # ── Textural blend ratio ───────────────────────────────────────────────
    # FIX D: configurable blend between generated and warped image
    #   1.0 = pure generated (paper intent)
    #   0.5 = equal blend (training-stable default)
    'blend_ratio'        : 0.7,
    ...
    for root, _, files in os.walk(data_path):
        for fname in sorted(files):
            ...
            samples.append({
                ...
                'subject': subject_id, 'hand': hand,
                'spectrum': spectrum, 'iteration': iteration,
                'hand_id': f"{subject_id}_{hand}", 'dataset': 'casia',
            })
    return samples
...
class CASIADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            'img': self.transform(img), 'identity': s['identity'],
            'path': s['path'], 'hand_id': s['hand_id'],
            'spectrum': s['spectrum'], 'subject': s['subject'], 'hand': s['hand'],
        }
    ...
    for root, _, files in os.walk(data_path):
        for fname in sorted(files):
            ...
            if not (len(subj_id) == 3 and subj_id.isdigit()
                    and session in ("1", "2") and device in ("h", "m")
                    and hand in ("l", "r")
                    and len(iteration) == 2 and iteration.isdigit()):
                skipped += 1; continue
            samples.append({
                'path': os.path.join(root, fname),
                'subject': subj_id, 'hand': hand,
                'session': session, 'device': device, 'iteration': iteration,
                'hand_id': f"{subj_id}_{hand}", 'spectrum': device, 'dataset': 'mpd',
            })
    if skipped:
        print(f"[Data/MPD] Skipped {skipped} non-standard files.")
    return samples
...
class MPDDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            'path': s['path'], 'hand_id': s['hand_id'],
            'spectrum': s['spectrum'], 'subject': s['subject'], 'hand': s['hand'],
            'session': s['session'], 'device': s['device'],
        }
...
def create_dataloaders(cfg):
    ...
    if dataset == 'casia':
        all_samples = load_casia_samples(cfg.casia_data_path)
        ...
        all_samples = load_mpd_samples(cfg.mpd_data_path)
        print(f"[Data] Samples : {len(all_samples)}")
        identity_map, num_classes = build_identity_map(all_samples)
        train_s, test_s, _, _ = split_train_test(
            all_samples, identity_map, cfg.train_ratio, cfg.random_seed)
        ...
        raise ValueError(f"Unknown dataset '{dataset}'. Use 'casia' or 'mpd'.")
...
class IdentityEncoder(nn.Module):
    """Frozen ResNet-50 through layer3 → (B, 1024, 7, 7)."""
    def __init__(self):
        super().__init__()
        from torchvision.models import resnet50
...
class PalmGenerator(nn.Module):
    ...
    def forward(self, z, id_feat):
        z_mod  = self.dropout(self.style_mlp(z))
...
class PalmDiscriminator(nn.Module):
    def __init__(self):
        ...
        def block(in_c, out_c, bn=True):
            ...
            return layers
        self.net = nn.Sequential(
            *block(  3,  64, bn=False), *block( 64, 128),
...
class PalmGenerationNetwork(nn.Module):
    ...
    def generate_from_z(self, z, x_id):
        """
        z   : style code s — ctrl[:, 4:] from the control vector
        x_id: identity source — already x′ (geometrically warped) during
              recognition training; raw x2 during GAN pre-training
        """
        id_feat = self.identity_encoder(x_id)
...
class PalmRecognitionNetwork(nn.Module):
    ...
    def extract(self, x):
        f = self.backbone(x).flatten(1)
        f = self.bn1(f); f = self.fc(f); f = self.bn2(f)
        return f
...
class UnifiedAugmentationModule(nn.Module):
    def __init__(self, style_dim=16, img_size=112, use_generation=True,
                 blend_ratio=0.5):
        """
        blend_ratio : float in [0, 1]  — FIX D
            Mixing weight for the textural branch output:
                x_out = blend_ratio * x_gen + (1 - blend_ratio) * x′
            blend_ratio = 1.0  → pure generated image (paper intent)
            blend_ratio = 0.5  → equal mix with warped original (stable default)
        """
        super().__init__()
    ...
    def forward(self, x, ctrl, aug_type='both'):
        ...
        if aug_type in ('geometric', 'both'):
            ...
            x = self.spatial_tf(x, spatial_p)      # x is now x′

        if aug_type in ('textural', 'both') and self.use_generation:
            z_style = ctrl[:, 4:]                   # s only — never t
            x_gen   = self.gen_network.generate_from_z(z_style, x)
            # x here is x′ when aug_type='both' (geometric already applied)
            x = self.blend_ratio * x_gen + (1.0 - self.blend_ratio) * x
...
class AdversarialAugOptimizer:
    ...
    Paper alignments (all fixed in this version):
      FIX A — K passed per call: K=1 geo, K=2 tex  (paper §4.3)
      FIX B — α ~ N(0.1, 0.001) sampled each call   (paper §4.3)
      Paper — Fθ frozen during z update (F*θ, Fig 2c)
    """

    def __init__(self, aug_module, rec_net):
        self.aug         = aug_module
        self.rec         = rec_net
        self.b_sp        = 0.3     # projection bound: spatial dims
        self.b_st        = 1.0     # projection bound: style dims
        # FIX B: α distribution — paper §4.3: sampled from N(0.1, 0.001)
        self._alpha_mean = 0.1
        self._alpha_std  = 0.001 ** 0.5    # σ = √0.001 ≈ 0.0316

    def optimize(self, x, labels, z_init, aug_type='both', steps=2):
        """
        x        : (B′, 3, H, W)  augmented subset, size = aug_gamma * B
        labels   : (B′,)
        z_init   : (B′, 4 + style_dim)  warm-started from MomentumSampler (Eq. 5)
        aug_type : 'geometric' | 'textural' | 'both'
        steps    : K PGD iterations — caller passes cfg.pgd_steps_geo or _tex
        """
        x_det = x.detach()

        # FIX B: sample α ~ N(0.1, 0.001) once per optimize() call
        alpha = float(torch.clamp(
            torch.normal(mean=torch.tensor(self._alpha_mean),
                         std=torch.tensor(self._alpha_std)),
            min=1e-4, max=1.0))
...
class MomentumSampler:
    ...
    Samples for the aug subset only (size = aug_gamma * B).
    """
...
class GANPretrainer:
    """
    Trains the identity-preserving generation network (Figure 3 / Eq. 7).
    Loss weights: λ_KL=0.01, λ_GAN=1.0, λ_1=1.0, λ_ID=5.0
    """
    ...
    def train_batch(self, x):
        B  = x.size(0)
        x1 = x                                          # style source
        x2 = x[torch.randperm(B, device=x.device)]     # identity source (≠ x1)
        ...
        self.opt_D.zero_grad(); l_D.backward(); self.opt_D.step()

        # Generator step
        gen2, _, _, mu2, lv2 = self.gen(x1, x2)
        l_adv = -self.gen.discriminator(gen2).mean()
        l_rec = F.l1_loss(gen2, x1)
        l_kl  = -0.5 * (1 + lv2 - mu2.pow(2) - lv2.exp()).sum(1).mean()
        with torch.no_grad():
            fi_r = self.gen.identity_encoder(x2).mean([2, 3])
        ...
        self.opt_G.zero_grad(); l_G.backward(); self.opt_G.step()
...
class Evaluator:
    ...
    def verification(self, feats, ids, far_targets):
        ...
        for far in far_targets:
            ...
            results[f"TAR@FAR={far:.0e}"] = float((gen_scores >= thr).mean())
    ...
    def identification(self, feats, ids, specs):
        ...
        return {f"Rank-{k}": float(np.any(ranked[:, :min(k, len(Gid))] == Pid[:, None],
                                          axis=1).mean())
                for k in [1, 5, 10]}
    ...
    def evaluate(self, loader, far_targets):
        ...
        return {**ver, **idf,
                'num_samples': len(feats), 'num_identities': int(np.unique(ids).size),
                'num_genuine': len(gen),   'num_imposter': len(imp)}

    def print_results(self, res):
        print("\n" + "=" * 80)
        print("  EVALUATION RESULTS")
        print("=" * 80)
        print(f"  Samples: {res['num_samples']}   Identities: {res['num_identities']}")
        print(f"  Genuine pairs : {res['num_genuine']:,}")
        print(f"  Imposter pairs: {res['num_imposter']:,}")
        print("\n  Verification (TAR@FAR):")
        for k in sorted(res):
            if 'TAR' in k or k == 'EER': print(f"    {k:<28} {res[k]:.4f}")
        print("\n  Identification:")
        for k in sorted(res):
            if 'Rank' in k: print(f"    {k:<28} {res[k]:.4f}")
        print("=" * 80 + "\n")
...
class UAATrainer:
    def __init__(self, cfg):
        ...
        self.rec = PalmRecognitionNetwork(
            ...
            s=cfg.arcface_s, m=cfg.arcface_m).to(self.dev)

        # FIX D: pass blend_ratio from config
        ...
        self.opt   = optim.SGD(self.rec.parameters(), lr=cfg.lr,
                               momentum=0.9, weight_decay=cfg.weight_decay)
        self.sched = build_lr_schedule(
            self.opt, cfg.warmup_epochs, cfg.num_epochs, cfg.lr)

        # FIX A+B: no fixed step_size, no single pgd_steps
    ...
    def pretrain_gan(self):
        if not self.cfg.use_generation:
            print("[GAN] Skipping — use_generation=False"); return

        if self.cfg.gen_save_path and os.path.exists(self.cfg.gen_save_path):
            ...
            for k, v in ckpt.items():
                ...
                if v.shape != current_state[k].shape:
                    ...
                    filtered[k] = v
            print(f"[GAN]   Loaded: {len(filtered)}/{len(current_state)} keys")
            if skipped_shape:
                print(f"[GAN]   Shape mismatch on {len(skipped_shape)} keys — will re-init")
            self.aug.gen_network.load_state_dict(filtered, strict=False)
            if skipped_shape or len(filtered) < len(current_state):
                print(f"[GAN] Running {self.cfg.gan_finetune_epochs} fine-tune epochs.")
                GANPretrainer(self.aug.gen_network, self.dev,
                              lr=self.cfg.gen_lr * 0.1,
                              epochs=self.cfg.gan_finetune_epochs,
                              save_path=None).run(self.train_loader)
            else:
                print("[GAN]   All weights loaded — no fine-tuning needed.")
        else:
            GANPretrainer(self.aug.gen_network, self.dev,
                          lr=self.cfg.gen_lr,
                          epochs=self.cfg.gen_pretrain_epochs,
                          save_path=self.cfg.gen_save_path).run(self.train_loader)
    ...
    def train_epoch(self, epoch):
        ...
        for batch in pbar:
            ...
            # FIX C: select γ fraction of batch for augmentation (paper §4.3)
            aug_size   = max(1, int(B * self.cfg.aug_gamma))
            aug_idx    = torch.randperm(B, device=self.dev)[:aug_size]
            x_sub      = x[aug_idx]
            labels_sub = labels[aug_idx]

            # Eq. 5: momentum-sampled z^0 — only for the augmented subset
            z_geo  = self.s_geo.sample(aug_size, self.dev)
            z_tex  = self.s_tex.sample(aug_size, self.dev)
            z_init = torch.cat([z_geo, z_tex], dim=1)

            # Geometric PGD — FIX A: K=1 step (paper §4.3)
            if self.cfg.use_geometric:
                z_opt = self.pgd.optimize(
                    x_sub, labels_sub, z_init,
                    aug_type='geometric',
                    steps=self.cfg.pgd_steps_geo)
                self.s_geo.update(z_opt[:, :4])
            else:
                z_opt = z_init

            # Textural PGD — FIX A: K=2 steps (paper §4.3)
            if self.cfg.use_textural and self.cfg.use_generation:
                z_opt2 = self.pgd.optimize(
                    x_sub, labels_sub, z_opt,
                    aug_type='textural',
                    steps=self.cfg.pgd_steps_tex)
                self.s_tex.update(z_opt2[:, 4:])
                z_final = z_opt2
            else:
                z_final = z_opt

            # Apply augmentation to subset only
            with torch.no_grad():
                x_aug = self.aug(x_sub, z_final, aug_type='both')

            # Eq. 4: full original batch (B) + augmented subset (γ·B)
            x_all  = torch.cat([x, x_aug],         dim=0)
            lb_all = torch.cat([labels, labels_sub], dim=0)
    ...
    def save_checkpoint(self, epoch, best=False):
        ...
        state = {'epoch': epoch, 'rec_net': self.rec.state_dict(),
                 'aug_module': self.aug.state_dict(),
                 'optimizer': self.opt.state_dict(), 'best_tar': self.best_tar}
        path = f"checkpoints/uaa{'_best' if best else f'_ep{epoch+1}'}.pt"
    ...
    def train(self):
        ...
        self.pretrain_gan()
        print(f"\n{'='*80}\n  PHASE 2 — RECOGNITION TRAINING WITH UAA\n{'='*80}")
        for epoch in range(self.cfg.num_epochs):
            self.train_epoch(epoch)
            if (epoch + 1) % self.cfg.eval_freq == 0 or epoch == self.cfg.num_epochs - 1:
                self.validate(epoch)
...
if __name__ == '__main__':
    ...
    for k, v in sorted(results.items()):
        if isinstance(v, float): print(f"    {k:<32} {v:.4f}")
        else: print(f"    {k:<32} {v}")
    print("=" * 80 + "\n")
